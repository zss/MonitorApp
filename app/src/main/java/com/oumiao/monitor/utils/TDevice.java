...
public class TDevice {
    ...
    public static void gotoMarket(Context context, String pck) {
        if (!isHaveMarket(context)) {
            MonitorApp.showToast("你手机中没有安装应用市场！");
            return;
        }
        Intent intent = new Intent();
        intent.setAction(Intent.ACTION_VIEW);
    ...
    public static void copyTextToBoard(String value) {
        copyTextToBoard(value, R.string.copy_success);
    }

    public static void copyTextToBoard(String value, String message) {
        if (TextUtils.isEmpty(value))
            return;
        ClipboardManager clip = (ClipboardManager) BaseApplication.context()
                .getSystemService(Context.CLIPBOARD_SERVICE);
        clip.setText(value);
        MonitorApp.showToast(message);
    }

    public static void copyTextToBoard(String value, int message) {
        if (TextUtils.isEmpty(value))
            return;
        ClipboardManager clip = (ClipboardManager) BaseApplication.context()
                .getSystemService(Context.CLIPBOARD_SERVICE);
    ...
    public static int getNetworkType() {
        int netType = 0;
        ConnectivityManager connectivityManager = (ConnectivityManager) MonitorApp
